...
var valor = window.prompt('Digite seu salário')
valor.toLocaleString('pt-BR',{style:'currency',currency:'BRL'})
document.write(`Olá, <strong>${nome}</strong>! Seu nome tem ${nome.length} letras.<br/>`)
document.write(`Seu nome com maiusculas é ${nome.toUpperCase()}<br/>`)
document.write(`Seu nome em minúsculas é ${nome.toLowerCase()}`)
document.write(`Seu salário é ${valor.toLocaleString('pt-BR',{style:'currency',currency:'BRL'})}`)*/

/*let nome = prompt('Qual é o seu nome?')
let idade = Number(prompt('Qual dua idade?'))
document.write(`Seu nome é ${nome} e sua idade é ${idade} anos!`)*/
...
resultado = '' //variável resultado inicia-se como zero
...
if (imc <18.5){
    ...
    resultado = "Obesidade grau III (mórbida) Mas pode ser chamado(a) também de Gordola ou Flávio Dino!"
}

/*alert(`Seu IMC é ${imc.toFixed(2)} e sua avialiação mostra estágio de ${resultado}`)*/

/*document.write(`Olá, <strong>${nome.toUpperCase()}!</strong><BR/><BR/>Se seu peso é de <strong>${peso} kgs</strong> e sua altura é <strong>${altura.toFixed(2)} m</strong>,<br/>`)
document.write(`então, seu IMC está em <strong>${imc.toFixed(2)}</strong> e você está classificado como <strong>${resultado}</strong><br><br>`)

document.write('<strong>IMC Classificação:</strong><BR/><strong>Abaixo de 18,5</strong> = Magreza<BR/><strong>De 18,50 até 24,90</strong> = Peso normal<BR/><strong>De 25  até 29,90</strong> = Sobrepeso<BR/><strong>De 30 até 34,9O</strong> = Obesidade grau I<BR/><strong>De 35 até 39,90</strong> = Obesidade grau II<BR/><strong>De 40 ou mais</strong> = Obesidade grau III (mórbida) - Flávio Dino(ssauro')*/
...
//Funções e Objetos Pré-definidos
...
//Número aleatório - Array Function sem parâmetros
...
//Array Function um parâmetro

// Com parênteses (sempre funciona)
const dobro = (n) => n * 2

// Sem parênteses (só com um parâmetro)
const dobro = n => n * 2
...
//- n => n * 2 → recebe n, retorna n * 2
//- Com zero ou dois+ parâmetros, parênteses são obrigatórios
...
//- console.log() → exibe no console (não é alert)
...
//- includes("a") → verifica se tem a letra "a"
//- replaceAll("a", "o") → troca TODOS os "a" por "o"
//- Se não tiver "a", retorna o texto original
...
//- .trim().toUpperCase() → executa um método após o outro (encadeamento)
